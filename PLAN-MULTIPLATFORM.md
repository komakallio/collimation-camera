...
| 3 — portable app | **Code complete on branch `spike/sdl3-gpu` and running on Windows** against the simulator: menu bar, sidebar, live view with the HLSL stretch shader, the six HUD scenes on an ImGui draw list, the save dialog, the error modal, the window icon, and the log file. `stretch shader math` pins the stretch maths and `stretch shader copies` pins the two MSL strings to each other. CI builds `CollimationCamera` on both runners. Not done: the app has never run on macOS, no camera has been plugged in, and §9.8's screenshot comparison against the SwiftUI app is untouched. |
...
6. Shader changes: update all four copies — `StretchParams.apply`, `MetalRenderer.shaderSource`, `ShaderSource.metal`, `ShaderSource.hlslFragment` — and the
